...
def upload_video(folder):
    ...
    summary["summary"] = (
        summary["summary"].replace("视频摘要：", "").replace("视频简介：", "").strip()
    )
    ...
    for retry in range(5):
        try:
            ...
            tags = (
                ["YouDub", summary["author"], "AI", "ChatGPT"]
                + tags
                + [
                    "中文配音",
                    "科学",
            ...
            with open(
                os.path.join(folder, "bilibili.json"), "w", encoding="utf-8"
            ) as f:
